...
<task type="auto">
  ...
  <action>
    ...
    **4. Create `src/event_dedup/config/prefixes.yaml`:**
    ...
    NOTE: Prefixes in the YAML file should use the normalized form (umlauts expanded) since comparison happens after normalization. The `strip_prefixes` function receives already-normalized text.

    WAIT -- actually, the prefix stripping should happen BEFORE final normalization OR the prefixes in the config should be stored in their original German form and matched case-insensitively against the original text, then the RESULT gets normalized. Let me clarify the correct approach:

    The normalization pipeline order should be:
    1. Strip prefixes from the ORIGINAL title (case-insensitive match against original-form prefixes)
    2. Then normalize the result (lowercase, umlaut expansion, punctuation stripping)

    So `prefixes.yaml` should contain original German forms:
    ```yaml
    dash_prefixes:
      - "Nordwiler Narrenfahrplan"
      - "Wahlkampfstand Buendnis 90/Die Gruenen"
    ...
    And `strip_prefixes()` does case-insensitive matching against the input title BEFORE normalization.

    Then create a top-level function:

...
must_haves:
  truths:
    ...
    - "ScoringWeights logs a warning when weights do not sum to approximately 1.0"
  artifacts:
...
6. Create Alembic migration `config/alembic/versions/005_add_config_settings.py`:
   ...
   - Use `import sqlalchemy as sa` and `sa.JSON` (not `from sqlalchemy.dialects.postgresql import JSON`) in the migration file, consistent with the SQLAlchemy model, so SQLite tests work.
...
8. Add a Pydantic `model_validator` on `ScoringWeights` in `src/event_dedup/matching/config.py` that logs a warning (via structlog) if weights do not sum to approximately 1.0 (within 0.01 tolerance). Do not reject the update — just warn. This prevents nonsensical match scores when operators set invalid weights via the frontend. Example:
   ```python
   ...
   def warn_if_weights_dont_sum(self) -> "ScoringWeights":
       ...
       if abs(total - 1.0) > 0.01:
           import structlog
           structlog.get_logger().warning("scoring_weights_sum_mismatch", total=total)
       return self
   ```
  </action>
  <verify>
